...
@pytest.mark.asyncio
async def test_validate_all_pipeline_timeout(
    mock_cache, naming_brief, name_candidate
):
...
@pytest.mark.asyncio
async def test_validate_all_retry_exhausted(
    mock_cache, naming_brief, name_candidate
):
...
@pytest.mark.asyncio
async def test_build_context_hash(
    mock_cache, naming_brief
):
...
@pytest.mark.asyncio
async def test_get_channel_from_adapter(
    mock_cache
):
...
@pytest.mark.asyncio
async def test_convert_to_core_result(
    mock_cache, naming_brief, name_candidate
):
    ...
    adapter = MockValidationAdapter(
        ...
        validate_result=ValidationResult(
            status=ValidationStatus.AVAILABLE,
            details={"url": "https://example.com", "manual_review_url": "https://review.com"}
        )
    )
...
@pytest.mark.asyncio
async def test_create_unverifiable_result(
    mock_cache, name_candidate
):
...
@pytest.mark.asyncio
async def test_create_unverifiable_result_with_none(
    mock_cache
):
...
@pytest.mark.asyncio
async def test_should_retry_with_transient_error(
    mock_cache
):
    ...
    for error_msg in ["timeout", "network error", "connection refused", "502", "503", "504", "429", "408"]:
...
@pytest.mark.asyncio
async def test_should_retry_with_non_transient_error(
    mock_cache
):
...
@pytest.mark.asyncio
async def test_should_retry_with_available_status(
    mock_cache
):
...
@pytest.mark.asyncio
async def test_get_channel_from_adapter_global_trademark(
    mock_cache
):
...
@pytest.mark.asyncio
async def test_get_channel_from_adapter_instagram(
    mock_cache
):
...
@pytest.mark.asyncio
async def test_get_channel_from_adapter_linkedin(
    mock_cache
):
...
@pytest.mark.asyncio
async def test_get_channel_from_adapter_unknown(
    mock_cache
):
...
@pytest.mark.asyncio
async def test_build_cache_key_with_different_contexts(
    mock_cache, name_candidate
):
