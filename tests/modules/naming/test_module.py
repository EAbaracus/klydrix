...
from launch_engine.modules.naming.brief import NamingBrief, NameTypology, PhoneticConstraints
from launch_engine.modules.naming.candidates import NameCandidate, NameCandidateList, InternalAssessment
...
async def test_run_returns_namelist(naming_module, mock_llm_adapter, sample_brief):
        ...
        json.dumps([
            {
                ...
                "tagline_options": ["Innovate with Novatek", "Novatek: Tech Forward"],
    ...
    assert len(result.candidates) == 2  # Limited by candidate_count=5 but we only have 2
...
@pytest.mark.asyncio
async def test_divergent_generation_with_mocked_llm(naming_module, mock_llm_adapter, sample_brief):
...
@pytest.mark.asyncio
async def test_convergent_scoring_with_mocked_llm(naming_module, mock_llm_adapter, sample_brief):
...
@pytest.mark.asyncio
async def test_error_handling_when_llm_fails(naming_module, mock_llm_adapter, sample_brief):
...
def test_prefilter_validates_and_cleans(naming_module):
    ...
    brief = NamingBrief(
        ...
        preferred_typologies=[NameTypology.INVENTED, NameTypology.INVENTED]  # Duplicate
    )
    ...
    assert set(brief.avoid_terms) == {"test", "bad"}  # Lowercased, deduplicated, empty removed
